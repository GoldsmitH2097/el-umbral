...
| anatomia | Anatomía del Vacío | arlequin | coming-soon | Interactive web experience. Since 2026-09-08 it renders in the same large **cofre** as the books: invite «Experiencia inmersiva» (`cofreInvite`), four dimmed **llaves** (card suits `picas/corazones/diamantes/treboles`, `llave:true` in retailers.js, stroke SVGs `palo-*.svg`) and the note «próximamente». Placeholders for the future paid access — «we play later with the access». Ghost SEO skips llaves (they are not shops). |
...
- **Indexed (as of July 27, 2026):** **11 indexed**, 23 not-indexed across 6 reasons. Of the 23, only 2 were a real defect (see "404 / soft-200" below); the rest are healthy-by-design (8 `Page with redirect`, 4 `Excluded by noindex`) or **stale crawls** Google hasn't refreshed — 4 `Server error (5xx)` last crawled Jan–Feb 2026 (pre-Vite site) and 3 `Redirect error` last crawled May 14–15 (the trailing-slash migration). All 7 stale URLs verified returning correct 301/200 today.
- **Sitemap:** `/sitemap.xml` ✅ 20 URLs (10 ES + 10 EN), status Success, 0 errors. A junk second entry (the homepage submitted *as* a sitemap in Dec 2025 — "Sitemap is HTML", 0 pages, permanent error) was removed from GSC on July 27, 2026. Only `/sitemap.xml` should ever be listed. Each URL carries bidirectional `xhtml:link hreflang` alternates. Legal pages intentionally excluded — they carry `noindex`.
...
- **404 / soft-200 (FIXED July 27, 2026 — do not regress):** the catch-all used to be `/* /index.html 200`, so **every** nonexistent URL returned HTTP 200 serving the homepage shell with `canonical="/"`. Google filed each as *"Alternate page with proper canonical tag"* — an unbounded set of phantom duplicates that no fix-validation could ever clear (confirmed victims: `/es`, `/index.html`). Now `/* /404.html 404` with a branded `public/404.html`. **This is only safe because all 20 routes are prerendered static files** — if you ever add a client-only route, prerender it in `generate-og-pages.js` or it will 404. Also added: `/es`+`/es/*` → `/`; `/index.html` → `/` and `/en/index.html` → `/en/` (`301!`); and 8 cross-language slug redirects (`/harlequin` → `/arlequin/`, `/en/arlequin` → `/en/harlequin/`, …) because `Router.js` resolves *both* slug sets on *either* prefix.
- **noindex vs canonical:** `anatomia.html` (`/obras/anatomia-del-vacio/leer`) carries `noindex, follow` and **deliberately no `rel=canonical`**. Google treats the pair as contradictory and may follow the canonical, risking the `noindex` carrying over to the antechamber — a page that must stay indexed. Don't re-add it.
- **robots.txt:** ✅
